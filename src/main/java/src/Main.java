...
import Exceptions.ReportService;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            try {
                switch (command) {
                    ...
                    case "rep_show":
                        ...
                        if (rShow.getSignedBy() != null) System.out.println("signed by: " + rShow.getSignedBy()); //если объект мпетода возвращает значение не равное нулю,то сисетма выводит значение
                        break;
                    ...
                    case "rep_updateline":
                        ...
                        long lineIdUpdate = Long.parseLong(parts[1]); //преобразование в чисдл
                        String[] part = parts[2].split("="); //Эти строки разбивают строку вида "ключ=значение" на две отдельные переменные
                        String field = part[0];
                        ...
                        if (field.equals("param")) {
                            newParam = MeasurementParam.valueOf(parts[1].toUpperCase()); //UpperCase-делает все букввы ЗАГЛАВНЫМИ valueOf-Ищет в перечислении (enum) константу с точно таким же именем и возвращает её.
                    ...
                    case "rep_export":
                        if (parts.length < 2) throw new ValidationException("Ошибка: укажите report_id");
                        long reportIdExport = Long.parseLong(parts[1]); //реобразователь строки в числo( parseLong)
            ...
            } catch (NumberFormatException e) { //нужен, чтобы поймать ошибку (исключение) и сделать с ней что-то разумное, вместо того чтобы программа просто упала.
