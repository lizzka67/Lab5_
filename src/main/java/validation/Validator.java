...
public class Validator {
    ...
    public static void validateReport(Report report) {
        ...
        if (report.getName().length() > 128) {
            throw new ValidationException("Ошибка: Название отчета слишком длинное (макс. 128 символов).");
        }
    }
    ...
    public static void validateReportLine(ReportLine line) {
        ...
        if (line.getUnit().length() > 16) {
            throw new ValidationException("Ошибка: Название единиц измерения слишком длинное (макс. 16 символов).");
        }
    }
}